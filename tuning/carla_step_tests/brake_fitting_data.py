"""
CARLA data collection for brake-to-deceleration mapping.

What it does
  - Drives to operating points and applies brake perturbations.
...
Run (example)
  python3 tuning/carla_step_tests/brake_fitting_data.py -f brake_out.csv
"""
...
def _fmt_brake_value(v):
    s = f"{v:.3f}".rstrip("0").rstrip(".")
    return s.replace(".", "p")


def _fmt_brake_pct(v):
    return f"{int(round(float(v) * 100.0)):03d}"
...
def main(args, img_queue, char_queue, img_lock):
    print("CARLA longitudinal brake identification test (synchronous mode)")
    ...
    brake_bins = list(BRAKE_BINS)
    baseline_brakes = [0.5 * (lo + hi) for (lo, hi) in brake_bins]

    os.makedirs(args.out_dir, exist_ok=True)

    # PID gains (same as accel_fitting_data.py)
    kp = 2.59397071e-01
    ki = 1.27381733e-01
    kd = 6.21160744e-03
    N_d = 5.0
    ...
    # Brake pulse configuration
    pre_step_time_s = float(args.pre_step_time_s)
    pre_step_steps = int(pre_step_time_s / dt)
    pulse_time_s = args.pulse_time_s
    pulse_steps = int(pulse_time_s / dt)
    num_perturbations = args.num_perturbations

    # Teleport to stay in a safe road segment
    ...
    total_pulses = len(speed_operation_points) * len(brake_bins) * int(num_perturbations)
    done_pulses = 0
    try:
        for setpoint_idx, sp_mps in enumerate(speed_operation_points):
            ...
            while stable_steps < stable_steps_required:
                ...
                if step % 50 == 0:
                    print(
                        ...
                        f"throttle={ref_throttle:.3f}, stable={stable_steps}/{stable_steps_required}"
                    )
            ...
            for (bin_lo, bin_hi), base_brake in zip(brake_bins, baseline_brakes):
                bin_lo = float(max(0.0, min(1.0, bin_lo)))
                bin_hi = float(max(0.0, min(1.0, bin_hi)))
                if bin_hi < bin_lo:
                    bin_lo, bin_hi = bin_hi, bin_lo
                base_brake = 0.0
                lo_tag = _fmt_brake_pct(bin_lo)
                hi_tag = _fmt_brake_pct(bin_hi)
                out_name = f"{args.froot}_{sp_kmh}kmh_{lo_tag}_{hi_tag}.csv"
                out_path = os.path.join(args.out_dir, out_name)
                print(
                    f"--- Brake bin [{bin_lo:.3f}, {bin_hi:.3f}] (random within bin, pre-step {pre_step_time_s:.2f}s) -> {out_path} ---"
                )
                ...
                with open(out_path, mode="w", newline="") as f:
                    ...
                    for pulse_idx in range(num_perturbations):
                        ...
                        print(
                            ...
                            f"sp={sp_kmh} km/h, bin=[{bin_lo:.3f},{bin_hi:.3f}], pulse={pulse_idx+1}/{num_perturbations}"
                        )
                        # Re-stabilize (PID) before each pulse so each test starts from
                        # the operating point. Capture the current "baseline throttle".
                        pid = PID(kp, ki, kd, N_d, dt, 0.0, 1.0, kb_aw, True, False)
                        stable_steps = 0
                        while stable_steps < stable_steps_required:
                            t_abs, speed, accel_x = tick_and_update_accel()
                        ...
                        # Record a 0-throttle, 0-brake pre-step window so the
                        # recorded input contains the step (u=0 -> u=brake_cmd).
                        vel_before_experiment = vehicle.get_velocity()
                        for _ in range(pre_step_steps):
                            t_abs, speed, accel_x = tick_and_update_accel()
                            sim_time = t_abs - start_timestamp
                        ...
                        # Pulse: choose a random brake command inside the bin.
                        brake_cmd = float(np.random.uniform(bin_lo, bin_hi))
                        brake_cmd = max(0.0, min(1.0, brake_cmd))
                        brake_delta = brake_cmd - base_brake
                        ...
                        for _ in range(pulse_steps):
                            ...
                            if (args.pulse_end_speed_mps > 0.0) and (
                                speed <= args.pulse_end_speed_mps
                            ):
                                print(
                                    f"Ending pulse early: speed={speed:.2f} m/s "
                                    f"<= {args.pulse_end_speed_mps:.2f} m/s"
                                )
                                break
                        ...
                        print(
                            ...
                            f"brake={brake_cmd:.3f} (bin [{bin_lo:.3f},{bin_hi:.3f}])"
                        )
                        if teleport_between_tests or pending_teleport:
                            pending_teleport = False
                            teleport_now_preserve_speed(vel_before_experiment)

        print("All brake tests completed.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CARLA brake identification test")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Duration of pre-step window [s] with throttle=0 and brake=0, logged before applying the brake step.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable teleporting back to the spawn transform between brake pulses.",
    )
    parser.add_argument(
        "--teleport-dist",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="End the brake pulse early if speed drops below this value [m/s]; set <= 0 to disable.",
    )
    parser.add_argument(
        "--no-lane-keep",
        action="store_true",
