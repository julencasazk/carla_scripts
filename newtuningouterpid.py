#!/usr/bin/env python3
"""
pso_outer_pid_carla_reuse_actors.py

PSO tuning of OUTER longitudinal PID in cascaded architecture, reusing the same 8 CARLA actors forever.
NO actor destruction in resets (only teleport + velocity reset + controller reset), matching the style of the inner-PID tuner.

Architecture:
  Outer PID: v_ref -> a_des (with saturation + jerk/rate limiting + antiwindup)
  Inner PID(s) (fixed): a_des -> u -> throttle/brake
    - single mode: one inner PID with u in [-1,1]
    - split mode:  two inner PIDs
        * throttle inner PID: u in [0,1]
        * brake inner PID:    u in [-1,0]

World:
  Town04, synchronous, fixed dt=0.01, physics substep 0.001 (max_substeps=10)
Default:
  no_rendering_mode=True (fastest), no walltime pacing.

Particles / batches ("tandas"):
  - 8 physical slots (spawnpoints): [115,116,117,118,79,80,81,82]
  - If particles > 8: evaluate in batches of 8 per iteration.
  - Each batch reuses the same 8 actors; for a smaller final batch, only first N slots are active (rest parked).

Per particle evaluation:
  For each scenario:
    1) Teleport active slots to their spawn (soft reset, no destroy).
    2) Warmup to v_op using OLD scheduled speed PID (from PID.py). Wait for band+hold.
    3) Run closed-loop (outer+inner) with v_ref profile: v_op -> v_op+dv -> v_op.
    4) Compute scenario cost. Total = sum over scenarios.
...
Usage:
  python3 pso_outer_pid_carla_reuse_actors.py --particles 32 --iters 100 \
    --inner-kp 0.12 --inner-ki 0.03 --inner-kd 0.0 --inner-N 10.0

Optional dashcam (slower):
  python3 ... --render --dashcam-vehicle 0
"""
...
def main() -> None:
    ...
    g.add_argument(
        ...
        help="Use two inner PIDs: throttle (u∈[0,1]) and brake (u∈[-1,0]).",
    )
    g.add_argument(
        "--no-split-inner",
        dest="split_inner",
        action="store_false",
        help="Use a single inner PID (u∈[-1,1]).",
    )
