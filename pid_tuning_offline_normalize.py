...
This script:
  1) Can compute robust scales (median+IQR) for each cost metric from
     random PID samples in the search space (offline scan).
  2) Uses those fixed scales to build a scalar cost:
         J = sum_j w_j * metric_j / sigma_j
     so each subcost has an absolute improvement gradient and similar
     magnitude, without manual _target/_max tuning.
...
def pso(objective,
        ...
        verbose=True,
        log_path=None):
    """Standard PSO minimizing scalar J(x)."""
    ...
    for i in range(num_particles):
        J, metrics = objective(particles[i])
        pbest_values[i] = J
