#!/usr/bin/env python3
"""
pso_inner_pid_carla.py

PSO tuning of INNER longitudinal PID:
    a_desired (m/s^2) -> u in [-1,1] -> throttle/brake

Key feature (requested):
- Only 8 spawn slots available.
- If --particles > 8, each PSO iteration evaluates the swarm in multiple batches of up to 8 particles:
    batches_per_iter = ceil(particles / 8)
  Reuses the same 8 vehicle actors:
    teleport reset -> warmup (old PID) -> inner test -> cost
  for each batch.
...
- substepping=True, max_substep_delta_time=0.001, max_substeps=10
- no_rendering_mode=True by default; use --render for dashcam

Spawnpoints:
    115,116,117,118,79,80,81,82

Warmup:
- Uses your old PID gains + scheduling (low/mid/high) from following_ros_cam.py via PID.py.
- Gate: do not start test until ALL ACTIVE vehicles stay within a generous speed band for warm_hold_s.

Inner test:
- a_des profile: 0 -> +a_plus -> 0 -> -a_minus -> 0
- accel measurement: derivative of longitudinal speed in vehicle frame + LPF

Notes:
- Put PID.py in same folder as this script.
"""
...
from PID import PID  # your old PID class (PID.py next to this script)
