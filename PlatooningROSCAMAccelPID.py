"""
ROS2 platoon member node (CARLA-agnostic) implementing a 2-loop longitudinal controller:

Outer loop:
  speed_setpoint -> desired_accel (m/s^2) via PID

Inner loop (single PID):
  desired_accel -> signed effort u in [-1, 1]
    u >= 0 => throttle = u, brake = 0
    u <  0 => throttle = 0, brake = -u

This mirrors the basic structure of CARLA's Ackermann controller, but remains
CARLA-agnostic: all simulator specifics are handled in the bridge node.

Topics (same scheme as PlatooningROSCAM.py):
  Subscribes:
    /<name>/state              : VehicleState (speed, accel, dist_to_prev, pose)
    /<name>/state/setpoint     : Float32 (base setpoint [m/s])
    /platoon/<platoon_id>/setpoint : Float32 (global platoon setpoint [m/s])
    /veh_<idx>/cam             : CAM (cooperative state from preceding vehicle)
  Publishes:
    /<name>/command/throttle   : Float32 [0..1]
    /<name>/command/brake      : Float32 [0..1]
    /<name>/state/local_setpoint : Float32 (effective speed setpoint [m/s])
    /veh_<idx>/cam             : CAM (cooperative state to followers)

Placeholder gains are used by default; tune them externally.
"""
