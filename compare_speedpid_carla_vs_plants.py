#!/usr/bin/env python3
"""
Run the same speed setpoint test:
  1) In CARLA (single vehicle, gain-scheduled speed PID -> throttle/brake)
  2) Offline using identified plants (control library) for low/mid/high ranges

Writes BOTH runs to the SAME CSV so you can overlay them with plot_csv.py.

Notes / assumptions:
  - This is the "simple" speed controller: a single PID per speed range that
    outputs a signed effort u. With u_min=0, it becomes throttle-only
    (brake stays 0.0); down-steps are handled by reducing throttle.
  - The offline plant is the incremental (Δu -> Δv) model around (u0, v0).
"""
